...
void Outline_float(float2 UV, out float4 Out)
{
    ...
    #endif  // OUTLINE_ONLY
    float depth = saturate(SampleDepth(UV));
    ...
    output.a = original.a;
    outputcolordepth.rgb = lerp(output, original, depth);
    outputcolordepth.a = original.a;

    #ifdef _COLORDEPTHOUTPUT
        Out = outputcolordepth;
    #else   
        Out = output;
    #endif
}
